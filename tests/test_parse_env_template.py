...
def test_underscores_in_variable_name():
    ...
    result = parse_env_template(template)
    assert result == ["VAR_WITH_UNDERSCORES"], "Expected to extract variable name with underscores"


def test_mixed_template():
    template = "Mixed template with variables $VAR1/something, text, and blah$VAR2."
    result = parse_env_template(template)
    assert result == ["VAR1", "VAR2"], "Expected to extract all variable names while ignoring plain text"
...
def test_all_available_with_all_variables_present():
    ...
    result = all_available(template)
    assert result is True, "Expected True when all variables are present in the environment"
    # Clean up
    del os.environ["VAR1"]
    del os.environ["VAR2"]
...
def test_all_available_with_missing_variable():
    ...
    template = "Template with $VAR1 and $VAR2."
    result = all_available(template)
    assert result is False, "Expected False when a variable is missing from the environment"
    # Clean up
    del os.environ["VAR1"]
